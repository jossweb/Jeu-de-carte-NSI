...
def main_welcome_page():
    ...
    button = style.set_button_setting(lambda: redirect_credits(window), window, "json/style_button_credits.json")
    canvas.create_window(30, 370, window=button)
    window.mainloop()

def redirect_credits(window):
    """cette fonction sert a rediriger vers la fonction de la fenêtre credits lors du clique
    sur le bouton associé a cette usage"""
    window.destroy()
    credits_page()
